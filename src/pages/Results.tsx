...
export const Results: React.FC = () => {
  ...
  // Auto-save project on first load
  useEffect(() => {
    if (user) {
      saveProject(user.id, {
        municipalityName,
  ...
  const grossYearlySavings = monthlySavings * 12;

  // Digitalization costs
  const totalOneTimeCosts = digitalizationCosts.implementationCost + digitalizationCosts.trainingCost;
  const totalAnnualCosts = digitalizationCosts.licenseCostYear + digitalizationCosts.maintenanceCostYear + digitalizationCosts.otherCostYear;
  const amortizedOneTime = totalOneTimeCosts / 5; // Spread over 5 years
  const totalYearlyCosts = totalAnnualCosts + amortizedOneTime;

  const yearlySavings = grossYearlySavings - totalYearlyCosts;
  const fiveYearSavings = (grossYearlySavings * 5) - (totalAnnualCosts * 5) - totalOneTimeCosts;
  ...
  const projectionData = Array.from({ length: 5 }, (_, i) => ({
    year: `Jahr ${i + 1}`,
    savings: (grossYearlySavings * (i + 1)) - (totalAnnualCosts * (i + 1)) - totalOneTimeCosts,
  }));
  ...
  const handleDownloadPdf = () => {
    ...
    doc.setFontSize(16);
    doc.text('Wichtigste Ergebnisse', 20, 50);
    ...
    doc.text(`${t.hoursFreedMonth}: ${hoursFreedPerMonth.toFixed(1)} Stunden`, 20, 90);
    doc.text(`${t.fte}: ${fteFreed.toFixed(2)} VZÄ`, 20, 96);
    ...
    <Layout>
      <div className="space-y-8 animate-fade-in pb-12">
        ...
        <div className="flex justify-between border-t border-hb-line/50 pt-8">
          <button
            onClick={() => navigate('/digitalization-costs')}
